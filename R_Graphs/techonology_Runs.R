...
grouped_data$Model <- ifelse(grouped_data$Run < 20000 & grouped_data$Platform=='PACBIO_SMRT', 'Another PACBIO_SMRT', grouped_data$Model)
grouped_data$Model <- ifelse(grouped_data$Run < 20000 & grouped_data$Platform=='ION_TORRENT', 'Another ION_TORRENT', grouped_data$Model)

tmp <-data.frame(
  Platform = 'PACBIO_SMRT',
  Model = 'Another PACBIO_SMRT',
  Run=sum(grouped_data$Run[grouped_data$Model == 'Another PACBIO_SMRT'])
)
...
tmp <-data.frame(
  ...
  Run=sum(grouped_data$Run[grouped_data$Model == 'Another ION_TORRENT'])
)
grouped_data <- grouped_data %>%
...
grouped_data$Model <- factor(grouped_data$Model, levels = rev(c("GridION",'MinION','PromethION','.','Sequel II',' RS II','Sequel',' RS',"Another PACBIO_SMRT" ,'..',"  PGM" ,"  Proton"," GeneStudio S5 Prime","  S5" ,"  S5 XL","Another ION_TORRENT")))
ggplot(grouped_data, aes(x = as.factor(Model), y = Run,fill=Platform)) +
  ...
  coord_flip() +
  labs(x = "Platform models", y = "Runs", title = "") +
  theme_minimal()+
  theme(
  ...
  scale_y_continuous(
    breaks = c(25000,100000,200000,300000,400000,500000,600000),
    labels = c('25k','100k','200k','300k','400k','500k','600k'),
    limits=c(0,750000)) 
